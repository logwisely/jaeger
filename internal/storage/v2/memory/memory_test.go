...
func TestGetIndexedAttributesNames(t *testing.T) {
	...
	rs.Resource().Attributes().PutStr("resource.attr", "r1")
	rs.Resource().Attributes().PutStr("@jaeger@internal", "skip")
	...
	span2.Attributes().PutInt("http.status_code", 200)

	require.NoError(t, store.WriteTraces(context.Background(), td))

	names, err := store.GetIndexedAttributesNames(context.Background(), tracestore.IndexedAttributesNamesQueryParams{
		Query: tracestore.TraceQueryParams{
			SearchDepth: 10,
		},
		Limit: 2,
	...
	require.NoError(t, err)
	assert.Equal(t, []string{"http.method", "http.status_code"}, names)
}

func TestGetKAttributeValues(t *testing.T) {
	store, err := NewStore(Configuration{
		MaxTraces: 10,
	})
	require.NoError(t, err)
	...
	for i := range traceIDs {
		td := ptrace.NewTraces()
		spans := td.ResourceSpans().AppendEmpty().ScopeSpans().AppendEmpty().Spans()
		for _, method := range methodsByTrace[i] {
			span := spans.AppendEmpty()
			span.SetTraceID(fromString(t, traceIDs[i]))
	...
	top, err := store.GetTopKAttributeValues(context.Background(), query)
	require.NoError(t, err)
	assert.Equal(t, []string{"GET", "DELETE"}, top)

	bottom, err := store.GetBottomKAttributeValues(context.Background(), query)
	require.NoError(t, err)
	assert.Equal(t, []string{"POST", "DELETE"}, bottom)
}
